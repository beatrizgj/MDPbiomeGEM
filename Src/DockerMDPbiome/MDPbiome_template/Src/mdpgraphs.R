...
mdpChrodGraph <- function(mdpTrans, actionVar, diroutput="", online=FALSE){
  ...
  for (iact in actions){
    ...
    #2019.11.20: BGJ: avoid error if not transition from some state
    empty.trans=FALSE
    istate=1
    while((!empty.trans)&&(istate<=length(states))){
      empty.trans= (sum(mat[istate,])==0)
      istate=istate+1
    }
    
    if(empty.trans){
      print(paste(iact,"chrodGraph not generated due to empty transitions!"))
    }else{
      if (!online){
...
actionEvalGraph <- function(freqs, policy, reward, actionVar, diroutput, graphname=""){
  ...
  pdf(paste(actiondir, graphname, tolower(actionVar),'_', reward, '_horizontal.pdf',sep=''))
  gbar <- ggplot(dataFreq, aes(x=state,y=stability, fill=action, alpha=as.factor(mark))) + 
    geom_bar(stat="identity") +
    coord_flip() +
    scale_alpha_manual(values=c("FALSE"=0.4, "TRUE"=1.0), guide='none') + 
    theme(panel.background = element_rect(fill = 'white'),
          text = element_text(size=24)) + 
    ggtitle(actionVar) +
    theme(legend.position="top") # BGJ: 2019.12.05: Legend position to reduce width bars
  print(gbar)
  dev.off()
}
...
followBarsGraph <- function(followResults, dirout, fname=NULL, glegend=TRUE){
  ...
  pdf(paste(dirout, paste(fname,'_horizontal.pdf',sep=''),sep=''))
  gobj <- ggplot(gdata, aes(x=Follow, y=Frequency, fill=Transition)) + 
    #geom_bar(stat = 'identity', position = 'stack') + 
    geom_bar(stat = 'identity') +
    coord_flip() +
    ...
    scale_fill_manual(values=c('#4daf4a','#377eb8','#e41a1c')) +
    facet_grid(~ Perturbation) + 
    ggtitle(titledata) + # BGJ: 2019.12.05: No title of dataset
    theme(legend.position="top") # BGJ: 2019.12.05: Legend position to reduce width bars
